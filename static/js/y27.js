        new Vue({
            ...
            methods: {
                ...
                createSeal11(id) {
                    ...
                    // 两边的符号
                    context.font = 'bolder ' + 20*this.sealScale09 + 'px 仿宋';
                    ...
                    for (var i = 0; i < counts; i++) {
                    	cs = charss[i];// 需要绘制的字符
                        if (i == 0){
                            context.rotate((4.6) * Math.PI / 6 - (counts - 1) * (angles - baseAngles) / 2); // centered bottom
                        }else{
                            context.rotate(angles);
                        }  
                        context.save();
                        context.translate(60*this.sealScale09 + this.fontBorder05, 0);// 平移到此位置,此时字和x轴垂直   
                    ...
                    for (var i = 0; i < count; i++) {
                        c = chars[i]; // 需要绘制的字符   
                        if (i == 0)
                            context.rotate((6.95) * Math.PI / 6 - (count - 1) * (angle - 4 * Math.PI / (3 * count + 28)) / 2) // centered;
                        else
